...
def layer_generator(ACF, n=config.n, K=config.K):
    if config.method == "direct":
        ...
        ACF = ACF[:K]
        S = dct(ACF, type=1)
        T = dct(np.sqrt(S)/(2*(K - 1)), type=1)  # apply convolution theorem and solve for T
...
def calculate_ACFs(rng, layer_gen, samples, M, n):
...
if __name__ == "__main__":
    ...
    r = calculate_ACFs(rng, layer_gen, samples, M, config.n)
    t_plot = np.linspace(0, M, 100)
    plt.plot(t_plot, gen_layer_ACF(t_plot, h, config.N), color="black", lw=1)
